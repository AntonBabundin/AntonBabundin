...
//     *
//------------------------------------------------------------------------------
class gen_he_mu_punct_sq extends base_sq;
    `uvm_object_utils(gen_he_mu_punct_sq)
    //----
    ...
    //----
    logic unsigned [4 : 0]                      format_he_mu           = 5;
    ...
    //----
    extern function      new(string name = "gen_he_mu_punct_sq");
...
function gen_he_mu_punct_sq::new(string name = "gen_he_mu_punct_sq");
    super.new (name);
endfunction: new

task gen_he_mu_punct_sq::pre_body();
    ...
    void'(std::randomize(pkt_bw) with { pkt_bw inside  {[1:3]};
                                        pkt_bw <= sys_bw;
                                    });
    ...
    START_SYS_PATTERN = {1'b0, 1'b0, format_he_mu, 1'b0, 1'b0, 1'b0, 1'b0, 1'b1, 1'b0, ch_tx, nhtp_4ch, pkt_bw, sys_bw, mu_subband_punct_coeff, gamma_rotation_coeff};
    START_STF_PATTERN = {1'b0, 1'b0, format_he_mu, 1'b0, 1'b0, 1'b1, 1'b0, 1'b0, 1'b0, ch_tx, nhtp_4ch, pkt_bw, sys_bw, mu_subband_punct_coeff, gamma_rotation_coeff};
    START_LTF_PATTERN = {1'b0, 1'b1, format_he_mu, 1'b0, 1'b0, 1'b0, 1'b1, 1'b0, 1'b0, ch_tx, nhtp_4ch, pkt_bw, sys_bw, mu_subband_punct_coeff, gamma_rotation_coeff};
    END_LTF_PATTERN   = {1'b0, 1'b1, format_he_mu, 1'b0, 1'b0, 1'b0, 1'b0, 1'b0, 1'b0, ch_tx, nhtp_4ch, pkt_bw, sys_bw, mu_subband_punct_coeff, gamma_rotation_coeff};
    END_SYS_PATTERN   = {1'b0, 1'b0, format_he_mu, 1'b0, 1'b0, 1'b0, 1'b0, 1'b0, 1'b0, ch_tx, nhtp_4ch, pkt_bw, sys_bw, mu_subband_punct_coeff, gamma_rotation_coeff};

    run_ref_model_for_chain(int'(sys_bw), int'(pkt_bw), int'(format_he_mu), 
    int'(gamma_rotation_coeff), int'(mu_subband_punct_coeff), int'(ch_tx), int'(nhtp_4ch));
endtask: pre_body


task gen_he_mu_punct_sq :: send_stf();
...
task gen_he_mu_punct_sq :: send_ltf();
    `uvm_create_on(trn, p_sequencer.m_dyn_pre_gen_sqr)
    fork
        begin            
            @(posedge $root.tb_hdl_top.clk);
