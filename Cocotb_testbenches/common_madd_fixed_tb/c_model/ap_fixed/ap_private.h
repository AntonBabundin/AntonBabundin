...
INLINE bool isNegative(const ap_private<_AP_W, true>& a)
{
    ...
    static const uint64_t sign_mask = 1ULL
                                      << ((_AP_W - 1) % APINT_BITS_PER_WORD);
...
#else
    static const unsigned Mod67Position[] = {64,
            0,
            1,
            39,
            2,
...
static INLINE bool add_1(
        uint64_t dest[], uint64_t x[], uint32_t len, uint64_t y)
{
    for (uint32_t i = 0; i < len; ++i)
    {
        dest[i] = y + x[i];
...
static INLINE bool add(uint64_t* dest,
        const uint64_t* x,
        const uint64_t* y,
        uint32_t destlen,
        uint32_t xlen,
...
static INLINE bool sub(uint64_t* dest,
        const uint64_t* x,
        const uint64_t* y,
        uint32_t destlen,
        uint32_t xlen,
...
static INLINE uint64_t mul_1(
        uint64_t dest[], const uint64_t x[], uint32_t len, uint64_t y)
{
    ...
    for (uint32_t i = 0; i < len; ++i)
    {
        ...
        carry = (((!carry && hasCarry != 2) || hasCarry == 1) ? two_power_32
                                                              : 0)
...
/// order to do a full precision computation. If it is not, then only the
/// low-order words are returned.
/// @brief Generalized multiplicate of integer arrays.
static INLINE void mul(uint64_t dest[],
        const uint64_t x[],
        uint32_t xlen,
        const uint64_t y[],
        uint32_t ylen,
        uint32_t destlen)
{
    ...
    for (uint32_t i = 1; i < ylen; ++i)
    {
        ...
        for (uint32_t j = 0; j < xlen; ++j)
        {
            ...
            if (i + j < destlen)
                dest[i + j] += resul;
            carry = (((!carry && hasCarry != 2) || hasCarry == 1) ? (1ULL << 32)
                                                                  : 0)
...
static INLINE void KnuthDiv(uint32_t* u,
        uint32_t* v,
        uint32_t* q,
        uint32_t* r,
        uint32_t m,
        uint32_t n)
{
    ...
    // u[i]); DEBUG(cerr << " by"); DEBUG(for (int i = n; i >0; i--) cerr << " "
    // << std::setbase(16) << v[i-1]); DEBUG(cerr << '\n');
    ...
    // D2. [Initialize j.]  Set j to m. This is the loop counter over the
    // places.
    int j = m;
    do
    {
        ...
        // qp by 1, inrease rp by v[n-1], and repeat this test if rp < b. The
        // test on v[n-2] determines at high speed most of the cases in which
        // the trial value qp is one too large, and it eliminates all cases
        // where qp is two too large.
        ...
        // consists of a simple multiplication by a one-place number, combined
        // with a subtraction.
        bool isNeg = false;
        for (uint32_t i = 0; i < n; ++i)
        {
            uint64_t u_tmp =
                    uint64_t(u[j + i]) | ((uint64_t(u[j + i + 1])) << 32);
        ...
        if (isNeg)
        {
            // D6. [Add back]. The probability that this step is necessary is
            // very small, on the order of only 2/b. Make sure that test data
...
void divide(const ap_private<_AP_W, _AP_S>& LHS,
        uint32_t lhsWords,
        ...
        ap_private<_AP_W, _AP_S>* Remainder)
{
    ...
    {
        // Now we're ready to invoke the Knuth classical divide algorithm. In
        // this case n > 1.
    ...
    if (Quotient)
    {
        ...
        if (lhsWords == 1)
        {
            uint64_t tmp = uint64_t(__Q[0])
                           | ((uint64_t(__Q[1])) << (APINT_BITS_PER_WORD / 2));
    ...
    if (Remainder)
    {
        ...
        // The remainder is in R. Reconstitute the remainder into Remainder's
        // low order words.
        if (rhsWords == 1)
        {
            uint64_t tmp = uint64_t(__R[0])
                           | ((uint64_t(__R[1])) << (APINT_BITS_PER_WORD / 2));
        ...
        ap_private<_AP_W, _AP_S>* Remainder)
{
    ...
    if (Remainder)
    {
        ...
        // The remainder is in __R. Reconstitute the remainder into Remainder's
        // low order words.
        if (rhsWords == 1)
        {
            uint64_t tmp = uint64_t(__R[0])
...
enum
{
    ...
    MAX_INT_BITS =
            (1 << 23) - 1  ///< Maximum number of bits that can be specified
                           ///< Note that this must remain synchronized with
...
/// values. It is a functional replacement for common case unsigned integer type
...
/// 15-bits, or more than 64-bits of precision. ap_private provides a variety of
  ...
  public:
    ...
    struct RType
    {
        enum
        {
            mult_w = _AP_W + _AP_W2,
            mult_s = _AP_S || _AP_S2,
            plus_w = AP_MAX(_AP_W + (_AP_S2 && !_AP_S),
                             _AP_W2 + (_AP_S && !_AP_S2))
                     + 1,
            plus_s = _AP_S || _AP_S2,
            minus_w = AP_MAX(_AP_W + (_AP_S2 && !_AP_S),
                              _AP_W2 + (_AP_S && !_AP_S2))
                      + 1,
            minus_s = true,
            div_w = _AP_W + _AP_S2,
            ...
            mod_s = _AP_S,
            logic_w = AP_MAX(
                    _AP_W + (_AP_S2 && !_AP_S), _AP_W2 + (_AP_S && !_AP_S2)),
    ...
    void operator=(const ap_private& RHS) volatile
    {
        ...
        VAL = RHS.get_VAL();  // No need to check because no harm done by
                              // copying.
    }
...
#undef CTOR
    ...
    INLINE ap_private<AP_MIN(64, _AP_W + 1), true> operator-() const
    {
    ...
    INLINE ap_private ashr(uint32_t shiftAmt) const
    {
        if (_AP_S)
            return ap_private(
                    (shiftAmt == BitWidth) ? 0 : ((int64_t)VAL) >> (shiftAmt));
        else
            return ap_private(
                    (shiftAmt == BitWidth) ? 0 : ((uint64_t)VAL) >> (shiftAmt));
    }
    ...
    template <int _AP_W2,
            ...
            int _AP_N2>
    INLINE ap_private(
            const af_bit_ref<_AP_W2, _AP_I2, _AP_S2, _AP_Q2, _AP_O2, _AP_N2>&
                    val)
    {
        set_canary();
            ...
            const ap_private<_AP_W1, _AP_S1>& RHS) const
    {
        ...
        {
            typename RType<_AP_W1, _AP_S1>::logic Ret(
                    ((uint64_t)VAL) & RHS.get_VAL());
            return Ret;
        }
        else
        {
...
#else
    INLINE ap_private operator&(uint64_t RHS) const
    {
    ...
    /// HiBits - This function returns the high "numBits" bits of this
    /// ap_private.
    ...
    INLINE void clearUnusedBits(void)
    {
        enum
        {
            excess_bits = (_AP_W % 64) ? 64 - _AP_W % 64 : 0
        };
        VAL = (ValType)(
                _AP_S ? ((((int64_t)VAL) << (excess_bits)) >> (excess_bits))
    ...
    INLINE bool ult(const ap_private<_AP_W1, _AP_S1>& RHS) const
    {
        ...
        {
            uint64_t lhsZext =
                    ((uint64_t(VAL)) << (64 - _AP_W)) >> (64 - _AP_W);
            uint64_t rhsZext = ((uint64_t(RHS.get_VAL())) << (64 - _AP_W1))
                               >> (64 - _AP_W1);
    ...
    INLINE static uint32_t getBitsNeeded(
...
#undef OP_BIN_AP
    ...
    INLINE typename RType<_AP_W2, _AP_S2>::div operator/(
            const ap_private<_AP_W2, _AP_S2>& op) const
    {
        ap_private<AP_MAX(_AP_W + (_AP_S || _AP_S2),
                           _AP_W2 + (_AP_S || _AP_S2)),
                (_AP_W > _AP_W2 ? _AP_S
            ...
            const ap_private<_AP_W2, _AP_S2>& op) const
    {
        ...
        typename RType<_AP_W2, _AP_S2>::mod res =
                typename RType<_AP_W2, _AP_S2>::mod(
                        _AP_S ? lhs.srem(rhs) : lhs.urem(rhs));
        return res;
    }
...
#undef OP_ASSIGN_AP_3_SINGLE
    ...
    INLINE bool operator==(const ap_private<_AP_W1, _AP_S1>& op) const
    {
        enum
        {
            _AP_MAX_W = AP_MAX(AP_MAX(_AP_W, _AP_W1), 32)
        };
    ...
    INLINE bool operator>(const ap_private<_AP_W2, _AP_S2>& op) const
    {
        ...
        {
            _AP_MAX_W = AP_MAX(
                    _AP_W + (_AP_S || _AP_S2), _AP_W2 + (_AP_S || _AP_S2))
        };
    ...
    {
        return ap_range_ref<_AP_W, _AP_S>(
    ...
    template <int _AP_W2, bool _AP_S2>
    INLINE ap_concat_ref<_AP_W,
            ap_private<_AP_W, _AP_S>,
            _AP_W2,
            ap_private<_AP_W2, _AP_S2>>
    concat(const ap_private<_AP_W2, _AP_S2>& a2) const
    {
        return ap_concat_ref<_AP_W,
                ap_private<_AP_W, _AP_S>,
                _AP_W2,
                ap_private<_AP_W2, _AP_S2>>(
                    ...
                    &a2) const
    {
        return ap_concat_ref<_AP_W,
                ...
                af_range_ref<_AP_W2, _AP_I2, _AP_S2, _AP_Q2, _AP_O2, _AP_N2>>(
                const_cast<ap_private<_AP_W, _AP_S>&>(*this),
                const_cast<af_range_ref<_AP_W2,
                        _AP_I2,
                        _AP_S2,
                        _AP_Q2,
                        _AP_O2,
...
template <int _AP_W, bool _AP_S>
std::string ap_private<_AP_W, _AP_S, true>::toString(
        uint8_t radix, bool wantSigned) const
{
    assert((radix == 10 || radix == 8 || radix == 16 || radix == 2)
            && "Radix should be 2, 8, 10, or 16!");
    static const char* digits[] = {"0",
            "1",
            "2",
            "3",
            "4",
    ...
    if (radix != 10)
    {
        ...
        else
        {
            ...
            {
                // They want to print the signed version and it is a negative
                // value Flip the bits and add one to turn it into the
                // equivalent positive value and put a '-' in the result.
                tmp.flip();
                tmp++;
                result = "-";
                insert_at = 1;
...
#pragma warning(disable : 4521 4522)
  ...
  private:
    ...
    /// ap_private
    /// @brief Construct an ap_private of numBits width, initialized as
    /// bigVal[].
    ...
    /// interpretation stops when the first charater that is not suitable for
    /// the radix is encountered. Acceptable radix values are 2, 8, 10 and 16.
    /// It is an error for the value implied by the string to require more bits
    /// than numBits.
    /// @param numBits the bit width of the constructed ap_private
    ...
    /// radix values are 2, 8, 10 and 16. It is an error for the value implied
    /// by the string to require more bits than numBits.
    /// @param numBits the bit width of the constructed ap_private
    ...
    INLINE void report()
    {
        ...
        {
            fprintf(stderr,
                    "[E] ap_%sint<%d>: Bitwidth exceeds the "
  ...
  public:
    ...
    ap_private(uint64_t* val, uint32_t bits = _AP_W)
    {
        assert(0);
    }
    ap_private(const uint64_t* const val, uint32_t bits)
    {
        assert(0);
    }
    ...
    /// will be done. Otherwise, no sign extension occurs (high order bits
    /// beyond the range of val are zero filled).
    /// @param numBits the bit width of the constructed ap_private
...
#undef CTOR
    ...
    /// This method is used internally to clear the to "N" bits in the high
    /// order word that are not used by the ap_private. This is needed after the
    /// most significant word is assigned a value to ensure that those bits are
    /// zero'd out.
    /// @brief Clear unused high order bits
    INLINE void clearUnusedBits(void)
    {
        pVal[_AP_N - 1] =
                _AP_S ? ((((int64_t)pVal[_AP_N - 1]) << (excess_bits))
                                >> excess_bits)
                      : (excess_bits ? ((pVal[_AP_N - 1]) << (excess_bits))
                                                >> (excess_bits)
                                     : pVal[_AP_N - 1]);
    }
    ...
    INLINE void fromString(const char* str, uint32_t slen, uint8_t radix)
    {
        ...
        // Set up an ap_private for the digit to add outside the loop so we
        // don't constantly construct/destruct it.
...
#undef isdigit
    ...
    {
        return ap_private_ops::lshr(
                ap_private_ops::shl(*this, _AP_W - numBits), _AP_W - numBits);
    }
...
#undef OP_BIN_LOGIC_ASSIGN_AP
    ...
    INLINE ap_private& operator+=(const ap_private<_AP_W1, _AP_S1>& RHS)
    {
        ...
        ap_private_ops::add(
                pVal, pVal, RHSpVal, _AP_N, _AP_N, _AP_N1, _AP_S, _AP_S1);
        clearUnusedBits();
        return *this;
    }
    ...
    ap_private& operator*=(const ap_private<_AP_W1, _AP_S1>& RHS)
    {
        ...
        ap_private_ops::mul(
                dest, pVal, lhsWords, dupRHS.get_pVal(), rhsWords, destWords);
...
#define OP_BIN_LOGIC_AP(Sym)                                                   \
    ...
    {                                                                          \
        ...
        {                                                                      \
            numWords =                                                         \
                    (RType<_AP_W1, _AP_S1>::logic_w + APINT_BITS_PER_WORD - 1) \
                    / APINT_BITS_PER_WORD                                      \
        };                                                                     \
...
#undef OP_BIN_LOGIC_AP
            ...
            const ap_private<_AP_W1, _AP_S1>& RHS) const
    {
        ...
        for (int i = 0; i < _AP_N1; ++i)
            RHSpVal[i] = RHS.get_pVal(i);
        bool carry = ap_private_ops::add(Result.get_pVal(),
                pVal,
                RHSpVal,
                (RType<_AP_W1, _AP_S1>::plus_w + 63) / 64,
                _AP_N,
                _AP_N1,
                _AP_S,
                _AP_S1);
        if ((RType<_AP_W1, _AP_S1>::plus_w + 63) / 64 > std::max(_AP_W, _AP_W1))
            Result.get_pVal((RType<_AP_W1, _AP_S1>::plus_w + 63) / 64 - 1) =
                    carry;
        Result.clearUnusedBits();
        return Result;
    }
            ...
            const ap_private<_AP_W1, _AP_S1>& RHS) const
    {
        ...
        for (int i = 0; i < _AP_N1; ++i)
            RHSpVal[i] = RHS.get_pVal(i);
        bool borrow = ap_private_ops::sub(Result.get_pVal(),
                pVal,
                RHSpVal,
                (RType<_AP_W1, _AP_S1>::minus_w + 63) / 64,
                _AP_N,
                _AP_N1,
                _AP_S,
                _AP_S1);
        if ((RType<_AP_W1, _AP_S1>::minus_w + 63) / 64
                > AESL_std::max(_AP_W, _AP_W1))
        {
            Result.get_pVal((RType<_AP_W1, _AP_S1>::minus_w + 63) / 64 - 1) =
                    borrow;
        }
        Result.clearUnusedBits();
        return Result;
...
#undef OP_ASSIGN_AP
            ...
            const ap_private<_AP_W3, _AP_S3>& LoIdx) const
    {
        ...
        int Lo = LoIdx.to_int();
        return ap_range_ref<_AP_W, _AP_S>(
                const_cast<ap_private*>(this), Hi, Lo);
    }
    ...
    INLINE bool isNegative() const
    {
        ...
        enum
        {
            shift = (_AP_W - APINT_BITS_PER_WORD * (_AP_N - 1) - 1)
        };
    ...
    /// This checks to see if the value has all bits of the ap_private are set
    /// or not.
    ...
    /// This checks to see if the value of this ap_private is the maximum
    /// unsigned value for the ap_private's bit width.
    ...
    /// This checks to see if the value of this ap_private is the minimum
    /// unsigned value for the ap_private's bit width.
    ...
    /// This function returns a pointer to the internal storage of the
    /// ap_private. This is useful for writing out the ap_private in binary form
    ...
    // the libc sqrt function is called. The result is rounded and then
    // converted back to a uint64_t which is then used to construct the result.
    // Finally, the Babylonian method for computing square roots is used.
    ap_private sqrt() const
    {
        ...
        if (magnitude <= 5)
        {
            static const uint8_t results[32] = {/*     0 */ 0,
                    /*  1- 2 */ 1,
                    1,
                    /*  3- 6 */ 2,
                    2,
        ...
        // If the magnitude of the value fits in less than 52 bits (the
        // precision of an IEEE double precision floating point value), then we
        // can use the libc sqrt function which will probably use a hardware
        // sqrt computation. This should be faster than the algorithm below.
        if (magnitude < 52)
        {
#ifdef _MSC_VER
        ...
        // Okay, all the short cuts are exhausted. We must compute it. The
        // following is a classical Babylonian method for computing the square
        // root. This code was adapted to APINt from a wikipedia article on such
        // computations. See http://www.wikipedia.org/ and go to the page named
        ...
        // off-by-one discrepancy with results from pari/gp. That discrepancy
        // has been determined to be a rounding issue with pari/gp as it begins
        // to use a floating point representation after 192 bits. There are no
        // discrepancies between this algorithm and pari/gp for bit widths < 192
        // bits.
    ...
    {
        ap_private Result(0);
        ap_private_ops::add(Result.get_pVal(),
                pVal,
                RHS.get_pVal(),
                _AP_N,
                _AP_N,
    ...
    {
        ap_private Result(0);
        ap_private_ops::sub(Result.get_pVal(),
                pVal,
                RHS.get_pVal(),
                _AP_N,
                _AP_N,
    ...
    ap_private ashr(uint32_t shiftAmt) const
    {
        ...
        // undefined. We return -1 if it was negative, 0 otherwise. We check
        // this early to avoid issues in the algorithm below.
        if (shiftAmt == BitWidth)
        {
            if (isNegative())
                return ap_private(-1);
        ...
        uint32_t offset =
                shiftAmt / APINT_BITS_PER_WORD;   // word offset for shift
        ...
        else
        {
            ...
            for (uint32_t i = 0; i < breakWord; ++i)
            {
                // This combines the shifted corresponding word with the low
                // bits from the next word (shifted into this word's high bits).
                val[i] = ((pVal[i + offset]) >> (wordShift));
                val[i] |= ((pVal[i + offset + 1])
                           << (APINT_BITS_PER_WORD - wordShift));
            }

            // Shift the break word. In this case there are no bits from the
            // next word to include in this word.
            val[breakWord] = (pVal[breakWord + offset]) >> (wordShift);

            // Deal with sign extenstion in the break word, and possibly the
            // word before it.
            if (isNegative())
            {
                if (wordShift > bitsInWord)
                {
                    if (breakWord > 0)
                        val[breakWord - 1] |=
                                ~0ULL << (APINT_BITS_PER_WORD
                                          - (wordShift - bitsInWord));
                    val[breakWord] |= ~0ULL;
                }
                else
    ...
    ap_private lshr(uint32_t shiftAmt) const
    {
        ...
        // undefined results. We define these "undefined results" to always be
        // 0.
        if (shiftAmt == BitWidth)
            return ap_private(0);
    ...
    ap_private shl(uint32_t shiftAmt) const
    {
        ...
        // If none of the bits are shifted out, the result is *this. This avoids
        // a lshr by the words size in the loop below which can produce
        // incorrect results. It also avoids the expensive computation below for
        // a common case.
        if (shiftAmt == 0)
            return *this;
    ...
    /// Perform an unsigned divide operation on this ap_private by RHS. Both
    /// this and RHS are treated as unsigned quantities for purposes of this
    /// division.
    ...
    ap_private udiv(const ap_private& RHS) const
    {
        ...
        // We have to compute it the hard way. Invoke the Knuth divide
        // algorithm.
        ap_private Quotient(0);  // to hold result.
        ap_private_ops::divide(
                *this, lhsWords, RHS, rhsWords, &Quotient, (ap_private*)0);
        return Quotient;
    }
    ...
    /// Perform an unsigned remainder operation on this ap_private with RHS
    /// being the divisor. Both this and RHS are treated as unsigned quantities
    /// for purposes of this operation. Note that this is a true remainder
    /// operation and not a modulo operation because the sign follows the sign
    /// of the dividend which is *this.
    ...
    ap_private urem(const ap_private& RHS) const
    {
        ...
        ap_private_ops::divide(
                *this, lhsWords, RHS, rhsWords, (ap_private*)(0), &Remainder);
        return Remainder;
    }
    ...
    INLINE bool ult(uint64_t RHS) const
    {
        ...
        uint32_t n1 = getActiveBits();
        uint32_t n2 = 64
                      - ap_private_ops::CountLeadingZeros_64(
                                RHS);  // RHS.getActiveBits();
    ...
    INLINE bool slt(const ap_private<_AP_W, _AP_S1>& RHS) const
    {
        ...
        // Now we have unsigned values to compare so do the comparison if
        // necessary based on the negativeness of the values.
        if (lhsNeg)
            if (rhsNeg)
                return lhs.ugt(rhs);
            else
    ...
    void cpSext(const ap_private<_AP_W1, _AP_S1>& that)
    {
        ...
        enum
        {
            wordBits = _AP_W1 % APINT_BITS_PER_WORD
        };
        const int _AP_N1 = ap_private<_AP_W1, _AP_S1>::_AP_N;
        ...
        if (_AP_N1 == _AP_N)
        {
            enum
            {
                newWordBits = _AP_W % APINT_BITS_PER_WORD
            };
            // The extension is contained to the wordsBefore-1th word.
            static const uint64_t mask =
                    wordBits ? (~0ULL << (wordBits)) : 0ULL;
            for (int i = 0; i < _AP_N; ++i)
    ...
    /// @returns the number of words to hold the integer value of this
    /// ap_private.
    ...
    /// extended uint64_t. The bitwidth must be <= 64 or the value must fit
    /// within a uint64_t. Otherwise an assertion will result.
    ...
    /// extended int64_t. The bit width must be <= 64 or the value must fit
    /// within an int64_t. Otherwise an assertion will result.
    ...
    static uint32_t getBitsNeeded(const char* str, uint32_t slen, uint8_t radix)
    {
        ...
        // For radixes of power-of-two values, the bits required is accurately
        // and easily computed
        if (radix == 2)
    ...
    /// countLeadingZeros_{32,64} functions in MathExtras.h. It counts the
    /// number of zeros from the most significant bit to the first one bit.
    ...
    {
        if (isSingleWord())
            return countLeadingOnes_64(
                    get_VAL(), APINT_BITS_PER_WORD - BitWidth);
    ...
    /// the radix given. The radix can be 2, 8, 10 or 16.
    ...
    INLINE double roundToDouble(bool isSigned) const
    {
        ...
        if (isSingleWord() || getActiveBits() <= APINT_BITS_PER_WORD)
        {
            ...
            {
                int64_t sext =
                        ((int64_t(val)) << (64 - BitWidth)) >> (64 - BitWidth);
                return double(sext);
            }
            else
                return double(val);
        ...
        // Number of bits in mantissa is 52. To obtain the mantissa value, we
        // must extract the high 52 bits from the correct words in pVal.
        uint64_t mantissa;
        ...
        if (hiWord == 0)
        {
            ...
            if (n > 52)
                (mantissa) >>=
                        (n - 52);  // shift down, we want the top 52 bits.
        }
        else
        {
            ...
            uint64_t lobits = (Tmp.get_pVal(hiWord - 1))
    ...
    /// re-interprets the bits of the double. Note that it is valid to do this
    /// on any bit width but bits from V may get truncated.
        ...
        const ap_private<AP_MAX(index, 1), true>& mark = 0)
{
    ...
    {
        APINT_BITS_PER_WORD = 64,
        word = index / APINT_BITS_PER_WORD
    };
...
template <int _AP_W, bool _AP_S>
std::string ap_private<_AP_W, _AP_S, false>::toString(
        uint8_t radix, bool wantSigned) const
{
    assert((radix == 10 || radix == 8 || radix == 16 || radix == 2)
            && "Radix should be 2, 8, 10, or 16!");
